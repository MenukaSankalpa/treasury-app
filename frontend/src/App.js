...
import { INIT_LOANS, INIT_DEPOSITS, INIT_IC, INIT_RATES, INIT_BALANCES } from "./mockData";
...
function MainApp() {
  ...
  return (
    <div style={{display:"flex",height:"100vh",fontFamily:"var(--font-sans)"}}>
      <div className="glass-sidebar" style={{width:230,display:"flex",flexDirection:"column",flexShrink:0}}>
        <div style={{padding:"20px 18px 16px"}}>
          ...
          <div style={{color:"#94A3B8",fontSize:11,marginTop:2}}>{USE_MOCK ? "Mock data mode" : "Acumatica integration ready"}</div>
        </div>
          ...
          {nav.map(key => {
            ...
            return (
              <button key={key} onClick={()=>setPage(key)} style={{
                ...
                background:isActive?"rgba(93,202,165,0.16)":"transparent",
                color:isActive?"#5DCAA5":"#94A3B8",
                ...
                {key==="notifications" && unread>0 && (
                  <span style={{marginLeft:"auto",background:"#A32D2D",color:"#fff",borderRadius:10,fontSize:10,padding:"1px 6px",fontWeight:700}}>{unread}</span>
                )}
              </button>
            );
          })}
        ...
        <div style={{padding:"14px 18px",borderTop:"0.5px solid rgba(255,255,255,0.08)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            ...
            <div style={{overflow:"hidden"}}>
              ...
              <div style={{color:"#64748B",fontSize:10}}>{user.empId} · {user.role}{user.company ? ` · ${user.company}` : ""}</div>
            </div>
          </div>
          <button onClick={logout} style={{width:"100%",padding:"7px",borderRadius:8,border:"0.5px solid rgba(255,255,255,0.1)",background:"transparent",color:"#94A3B8",fontSize:12,cursor:"pointer"}}>
            Log out
          </button>
        </div>
      </div>
