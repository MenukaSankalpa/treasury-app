...
export default function LoansPageCompany({loans, setLoans}) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const initialStatus = user.role === "Accountant" ? "Pending Company Head" : "Pending Accountant Review";
      const newLoan = {
        ...payload, id:"L"+Date.now(), status:initialStatus, cashBacked:false,
        requestedBy:user.name, requestedByEmail:user.email, editHistory:[],
        accountantDecidedBy: user.role === "Accountant" ? user.name : undefined,
        accountantDecidedDate: user.role === "Accountant" ? fmt(TODAY) : undefined,
      };
      ...
      if (USE_MOCK) {
        ...
        setLoans([...loans, {...created, status:initialStatus, requestedBy:user.name, requestedByEmail:user.email, attachments:form.attachments, editHistory:[]}]);
      }

      const notifyRole = user.role === "Accountant" ? "CompanyHead" : "Accountant";
      notify({
        title: user.role === "Accountant" ? "New facility needs Company Head approval" : "New facility needs Accountant review",
        message:`${payload.company} — ${fmtFull(payload.outstanding)} at ${payload.bank} needs ${notifyRole === "CompanyHead" ? "Company Head" : "Accountant"} review`,
        targetRoles:[notifyRole], entityId:newLoan.id,
      });
      ...
      const recipient = user.role === "Accountant"
        ? findUser(users, "CompanyHead", user.company)
        : findUser(users, "Accountant", user.company);
      await sendActionEmail({
        toEmail: recipient?.email, toName: recipient?.name || notifyRole,
        message: `A new facility request (${payload.bank} — ${fmtFull(payload.outstanding)}) needs your ${user.role === "Accountant" ? "approval" : "review"}.`,
        ...
        <Modal title="Add loan / facility" onClose={() => setModal(null)} wide>
          <div style={{background:"var(--color-background-info)",border:"0.5px solid var(--color-border-secondary)",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:13,color:"var(--color-text-info)"}}>
            {user.role === "Accountant"
              ? "This request will go directly to your Company Head, then Treasury for final approval."
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
            ...
            <button style={S.btn("#185FA5")} onClick={save} disabled={saving}>{saving ? "Submitting…" : "Submit for approval"}</button>
          </div>
        </Modal>
      )}
