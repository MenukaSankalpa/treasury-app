...
export default function IntercompanyCompany({icLoans, setIcLoans}) {
  ...
  const canRequest = canDoAction(user, "new_funding_request") && user.role === "TeamMember";
  ...
  const blankReq = {
    ...
    remarks:"", attachments:[], reviewedBy:"",
  };
  ...
  const submitRequest = async () => {
    ...
    try {
      ...
      const req = {
        ...
        steps:[
          {order:1, role:"TeamMember", company:user.company, status:"Approved", by:user.name, date:fmt(TODAY)},
          {order:2, role:"Accountant", company:user.company, status:"Pending"},
          {order:3, role:"CompanyHead", company:user.company, status:"Pending"},
      ...
      notify({title:"New IC loan request", message:`${req.requestNo} — ${fmtFull(req.amount)} needs Accountant review`, targetRoles:["Accountant"], entityId:id});
      ...
      await sendActionEmail({
        ...
        message: `${req.requestNo} — ${fmtFull(req.amount)} needs your review before it goes to Company Head.`,
  ...
  const accountantDecide = async (req, action, reason="") => {
    setActing(true);
    try {
      let updated;
      if (USE_MOCK) {
        ...
        updated = await intercompanyApi.accountantDecide(req.id, action, reason);
      }
      updateReqLocal(req.id, updated);
      notify({title:action==="approve"?"Approved — sent to Company Head":"Request rejected",
        message:`${req.requestNo} ${action==="approve"?"moved to Company Head review":"was rejected by Accountant"}`,
        targetRoles: action==="approve" ? ["CompanyHead"] : [], targetEmails:[req.requestedByEmail], entityId:req.id});
      logAction({user, action:`Accountant ${action}d request`, entityType:"IC Loan", entityId:req.requestNo});

      const users = await loadUsers();
      if (action === "approve") {
        const head = findUser(users, "CompanyHead", req.borrowerCompany);
        await sendActionEmail({
          toEmail: head?.email, toName: head?.name || "Company Head",
          message: `${req.requestNo} has been reviewed by Accountant and needs your approval.`,
        ...
        await sendActionEmail({
          ...
          message: `Your intercompany funding request ${req.requestNo} was rejected by Accountant. Reason: ${reason}`,
          loanRef: req.requestNo, borrower: req.borrowerCompany, amount: fmtFull(req.amount),
        });
      }

      setDetail(null); setChRejectOpen(false);
    } catch (err) {
      alert(err.message);
    } finally {
      setActing(false);
  ...
  const actOnBorrowerStep = async (req, action, reason="") => {
    ...
    try {
      let updated;
      if (USE_MOCK) {
        const steps = req.steps.map(s => s.order===3 ? {...s, status:action==="approve"?"Approved":"Rejected", by:user.name, date:fmt(TODAY)} : s);
  ...
  const lenderAct = async (req, action) => {
    ...
    try {
      ...
      if (updated.status === "Rejected") {
        ...
        const decision = req.treasuryDecision;
        const summary =
`All lenders have confirmed funding for ${req.requestNo}. Please review and release the payment.

Summary:
- Borrower: ${req.borrowerCompany}
- Bank: ${req.bankName} | Account: ${req.bankAccountNo}
- Loan type: ${req.loanType}
- Amount: ${fmtFull(req.amount)}
- Purpose: ${req.borrowerPurpose}
- Requested date: ${req.requestedDate}
- Repayment date: ${decision?.finalRepaymentDate || req.requestedRepaymentDate}
- Interest rate: ${decision ? fmtPct(decision.interestRate) : "—"}
- Handling fee: ${fmtFull(decision?.handlingFee || 0)}
- Lenders: ${decision?.lenders?.map(l=>`${l.company} (${fmtFull(l.amount)})`).join(", ") || "—"}
- Reviewed by: ${req.reviewedBy || "—"}`;
        await sendActionEmail({ toEmail: fc?.email, toName: fc?.name || "Finance Controller", message: summary, loanRef: req.requestNo, borrower: req.borrowerCompany, amount: fmtFull(req.amount) });
      }
  ...
  const settlementAct = async (req, role, action) => {
    ...
    try {
      ...
      if (updated.status === "Settled") {
        ...
        const users = await loadUsers();
        const fc = findUser(users, "FinanceController");
        await sendActionEmail({ toEmail: fc?.email, toName: fc?.name || "Finance Controller", message: `${req.requestNo} settlement is ready for your final confirmation.`, loanRef: req.requestNo, borrower: req.borrowerCompany, amount: fmtFull(req.amount) });
      }
  ...
  const initiateSettle = async () => {
    ...
    try {
      ...
      if (user.role !== "Accountant") {
        ...
        await sendActionEmail({ toEmail: accountant?.email, toName: accountant?.name || "Accountant", message: `${detail.requestNo} settlement has been initiated. Please confirm.`, loanRef: detail.requestNo, borrower: detail.borrowerCompany, amount: fmtFull(detail.amount) });
      } else {
        const head = findUser(users, "CompanyHead", detail.borrowerCompany);
        await sendActionEmail({ toEmail: head?.email, toName: head?.name || "Company Head", message: `${detail.requestNo} settlement has been initiated and confirmed by your Accountant. Please confirm.`, loanRef: detail.requestNo, borrower: detail.borrowerCompany, amount: fmtFull(detail.amount) });
      }
    ...
    <div>
      ...
      {newModal && (
        <Modal title="New intercompany funding request" onClose={()=>setNewModal(false)} wide>
          ...
          <div style={{background:"var(--color-background-info)",border:"0.5px solid var(--color-border-secondary)",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:13,color:"var(--color-text-info)"}}>
            This request will go to your Accountant first, then Company Head, then Treasury for lender assignment and final terms.
          </div>
          <FormRow>
          ...
          <Field label="Reviewed by (optional note)" value={form.reviewedBy} onChange={v=>setForm({...form,reviewedBy:v})} />
      ...
      {detail && (
        <Modal title={`${detail.requestNo} — ${detail.borrowerCompany}`} onClose={()=>setDetail(null)} wide>
          ...
          {detail.reviewedBy && <div style={{fontSize:12,color:"var(--color-text-secondary)",marginBottom:16}}><strong>Reviewed by:</strong> {detail.reviewedBy}</div>}
          ...
          {detail.treasuryDecision && (
            <GlassCard title="Treasury terms" style={{marginBottom:16}}>
              ...
              {canSeeHandlingFee(user, detail) && <div style={{fontSize:12,color:"var(--color-text-warning)"}}><strong>Handling fee:</strong> {fmtFull(detail.treasuryDecision.handlingFee||0)}</div>}
          ...
          {detail.settlement?.steps?.length > 0 && (
            <GlassCard title="Settlement approval" style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:10}}>
                {detail.settlement.steps.map((s,i) => <Badge key={i} type={s.status==="Approved"?"green":s.status==="Rejected"?"red":"amber"}>{s.role}: {s.status}</Badge>)}
              </div>
          ...
          {detail.status==="Pending Accountant Review" && canAccountantReview && user.company===detail.borrowerCompany && (
            <ActionBox label="Accountant review needed">
              <button style={S.btn("#0F6E56")} disabled={acting} onClick={()=>accountantDecide(detail,"approve")}>{acting?"…":"Approve"}</button>
          ...
          {detail.status==="Pending Settlement" && user.company===detail.borrowerCompany && (() => {
            ...
            const myStep = detail.settlement.steps.find(s => s.role===user.role && s.status==="Pending" && (s.role==="Accountant" || accountantStep.status==="Approved"));
      ...
      {settleOpen && detail && (
        <Modal title={`Settle ${detail.requestNo}`} onClose={()=>setSettleOpen(false)}>
          <p style={{fontSize:12,color:"var(--color-text-secondary)",marginBottom:14}}>
            This starts the settlement approval chain: your Accountant → Company Head → Finance Controller.
          </p>
          <SettlementBreakdown loan={detail} />
          ...
          <FileUpload files={settleForm.attachments} setFiles={f=>setSettleForm({...settleForm,attachments:f})} label="Settlement proof" />
          <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:8}}>
      ...
      {chRejectOpen && detail && (
        <Modal title={`Reject ${detail.requestNo}`} onClose={()=>setChRejectOpen(false)}>
          <p style={{fontSize:13,color:"var(--color-text-secondary)",marginBottom:16}}>This will reject the request and notify the requester, with the reason shown below.</p>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:12}}>
            ...
            <button
              ...
              onClick={async ()=>{
                ...
                if (detail.status === "Pending Accountant Review") {
                  await accountantDecide(detail,"reject",chRejectReason);
                } else {
