...
// Loads the real user directory once — mock list locally, real API when connected.
...
export default function IntercompanyCompany({icLoans, setIcLoans}) {
  ...
  const submitRequest = async () => {
    ...
    try {
      ...
      if (USE_MOCK) {
        ...
        created = await intercompanyApi.createLoan(req);
      ...
      // Real email to this company's Company Head
  ...
  const actOnBorrowerStep = async (req, action, reason="") => {
    ...
    try {
      ...
      if (action === "approve") {
        // Real email to Treasury — no company scope, they're a global role.
        const users = await loadUsers();
...
const lenderAct = async (req, action) => {
  ...
  try {
    ...
    if (USE_MOCK) {
      ...
      // Every single row — every lender company's Accountant AND Company Head — must be Approved.
    ...
    console.log("lenderAct result — new status:", updated.status, "| all confirmations:", updated.lenderConfirmations);
    ...
    if (updated.status === "Rejected") {
      ...
      // Every lender company (Accountant + Company Head, all of them) has now approved.
      // This is the ONLY point in the whole flow where Finance Controller gets emailed.
      const users = await loadUsers();
      const fc = findUser(users, "FinanceController");
      console.log("Attempting FC email —", fc);
      if (!fc) {
        console.error("No FinanceController user found in the directory — cannot email. Check MOCK_USERS or your users table.");
      }
      await sendActionEmail({
  ...
  const settlementAct = async (req, role, action) => {
    ...
    try {
      ...
      if (updated.status === "Settled") {
        ...
        // Company Head just confirmed settlement — Finance Controller's turn is now unlocked.
        const users = await loadUsers();
        const fc = findUser(users, "FinanceController");
        await sendActionEmail({
          toEmail: fc?.email, toName: fc?.name || "Finance Controller",
  ...
  const initiateSettle = async () => {
    ...
    try {
      ...
      // If the initiator wasn't the Accountant, notify the Accountant next in line.
      if (user.role !== "Accountant") {
        ...
        // Initiator was the Accountant — their step is auto-approved, so notify Company Head next.
        const users = await loadUsers();
        ...
        <Modal title={`${detail.requestNo} — ${detail.borrowerCompany}`} onClose={()=>setDetail(null)} wide>
          ...
          {detail.status==="Pending Company Head" && canCompanyHeadDecide && user.company===detail.borrowerCompany && (
            <ActionBox label="Company Head approval needed">
              ...
              <button style={S.btn("#A32D2D")} disabled={acting} onClick={()=>{const r=prompt("Reason for rejection:"); if(r!==null) actOnBorrowerStep(detail,"reject",r);}}>Reject</button>
            </ActionBox>
          )}
