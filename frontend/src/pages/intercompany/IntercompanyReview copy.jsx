...
export default function IntercompanyReview({icLoans, setIcLoans}) {
  ...
  const treasuryApprove = async () => {
    ...
    try {
      ...
      // Real emails to each lender company's Accountant (first confirmer in that chain)
      if (!isEditing) {
  ...
  const treasuryReject = async () => {
    ...
    try {
      ...
      if (USE_MOCK) {
        ...
        updated = {...detail, steps, status:"Rejected", rejectReason:reason};
      } else {
        updated = await intercompanyApi.treasuryDecide(detail.id, { action:"reject", reason });
      }
      updateReqLocal(detail.id, updated);
      notify({title:"Request rejected by Treasury", message:`${detail.requestNo}: ${reason}`, targetEmails:[detail.requestedByEmail], entityId:detail.id});
      logAction({user, action:"Treasury rejected request", entityType:"IC Loan", entityId:detail.requestNo, details:reason});

      // Real email to the original requester
      await sendActionEmail({
        toEmail: detail.requestedByEmail, toName: detail.requestedBy,
        message: `Your intercompany funding request ${detail.requestNo} was rejected by Treasury. Reason: ${reason}`,
    ...
    try {
      ...
      if (USE_MOCK) {
        if (action === "reject") {
          ...
          updated = {...req, financeController:{status:"Released", releasedBy:user.name, releasedDate:fmt(TODAY)}, status:"Active"};
        }
      } else {
        updated = await intercompanyApi.releasePayment(req.id, action, reason);
      }
      ...
      if (action === "approve") {
        ...
        logAction({user, action:"Finance Controller released payment", entityType:"IC Loan", entityId:req.requestNo});

        // Real email to Treasury confirming release, and to the original requester
        const users = await loadUsers();
        const treasury = findUser(users, "Treasury");
        await sendActionEmail({
          toEmail: treasury?.email, toName: treasury?.name || "Treasury",
          message: `${req.requestNo} payment has been released by Finance Controller — the loan is now Active.`,
  ...
  const initiateSettle = async () => {
    ...
    try {
      ...
      // Real email to the borrower company's Accountant — first in the settlement chain
      const users = await loadUsers();
        ...
        <Modal title={`${detail.requestNo} — ${detail.borrowerCompany}`} onClose={()=>setDetail(null)} wide>
          ...
          {detail.status==="Pending Treasury" && canTreasuryAct && (
            <ActionBox label="Treasury decision needed">
              ...
              <button style={S.btn("#A32D2D")} disabled={acting} onClick={treasuryReject}>Reject</button>
            </ActionBox>
          )}
          ...
          {detail.status==="Pending Finance Controller" && canRelease && (
            <ActionBox label="All lenders confirmed — release payment">
              <button style={S.btn("#0F6E56")} disabled={acting} onClick={()=>releasePayment(detail,"approve")}>{acting?"…":"Release payment"}</button>
